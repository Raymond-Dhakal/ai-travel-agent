...
with st.sidebar:
    ...
    travel_style = st.selectbox(
        ...
        index=["Budget", "Comfort", "Luxury"].index(st.session_state.travel_style),
        key="travel_style",
    )
...
with st.form("travel_form"):
    ...
    with col1:
        ...
        must_see = st.text_input(
            ...
            value=st.session_state.get("must_see", ""),
            placeholder="E.g., Louvre, Eiffel Tower",
            key="must_see",
        )
        avoid = st.text_input(
            "Avoid (optional)",
            value=st.session_state.get("avoid", ""),
            placeholder="E.g., long walks, museums",
            key="avoid",
        )
    with col2:
        start_date = st.date_input(
            "Start date",
            value=st.session_state.get("start_date", datetime.today().date()),
            key="start_date",
        )
        end_date = st.date_input(
            "End date",
            value=st.session_state.get("end_date", (datetime.today() + timedelta(days=5)).date()),
            key="end_date",
        )
        ...
        budget = st.slider(
            ...
            min_value=50,
            max_value=10000,
            value=st.session_state.get("budget", 800),
            step=10,
            key="budget",
        )
        pace = st.radio(
            ...
            index=["Relaxed", "Balanced", "Packed"].index(st.session_state.get("pace", "Balanced")),
            key="pace",
        )

    extra_notes = st.text_area(
        "Extra notes / constraints",
        value=st.session_state.get("extra_notes", ""),
        placeholder="e.g., wheelchair accessible, vegetarian food only",
        key="extra_notes",
    )
...
def build_prompt():
    ...
    parts.append(
        "Please produce a daily itinerary with transport suggestions, approximate cost breakdown, "
        "recommended restaurants or activities, and alternative options. Also include packing tips and "
        "links to useful maps/booking pages if possible."
    )
    return "\n\n".join(parts)
